...
For a while now I have been circling the same problem: how do you version _datasets_ when the data is large binaries?
...
What I want is content-defined chunking (split files on content boundaries so an edit only touches the chunks it changed) paired with global, content-addressed deduplication, so a new dataset version costs only its new bytes.
I have spent more time than I would like to admit surveying the options and never quite found a complete one.
...
Building it taught me the shape of the problem cold, but it also made the gap obvious: a CAS server is only the storage half, not the _version control_ half, and nothing in the Python data stack can read from one without a pile of bespoke client code.
...
Two guard rails hold that contract up.
