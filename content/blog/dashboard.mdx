...
For the user interface, I relied on vanilla HTML/CSS, Jinja templates, and DaisyUI[^1], a lightweight component library built on Tailwind CSS.
...
FastAPI makes serving static files simple:
...
The HTML templates were powered by Jinja2 Fragments[^2], which allowed partial rendering of templates by targeting specific block names. Combined with HTMX, this kept the template files short and easy to maintain.
...
This is safer than shelling out with subprocess or similar tools.
...
I also wrote a simple session management service so that only authorized users could reach the dashboard.
...
This project took less than a day to implement, thanks to how little FastAPI and HTMX ask of you. After some minor tweaks for mobile responsiveness, my lab members started using it.
